---
description: >-
  List open PRs on a target repo, walk the JSON queue with review-pr-queue.sh,
...
## Step 2: Walk the queue with `review-pr-queue.sh`

After `cache/pr-review/prs-to-review.json` exists, run `review-pr-queue.sh` from the project root:

```bash
./review-pr-queue.sh
```

If it is not executable, use `bash review-pr-queue.sh` instead.
